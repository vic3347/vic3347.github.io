...
OUTPUT_NAME = "/Users/vthompson/Downloads/North_America_All_Files"


def find_north_america_folder(base: Path) -> Path:
    ...
    matches = []
    for p in base.rglob("*"):
        if p.is_dir() and p.name.lower().replace("-", "_").replace(" ", "_") == "north_america":
            matches.append(p)
    ...
    if len(matches) > 1:
        print("More than one possible North America folder was found:")
        for p in matches:
            print(f"  {p}")
        sys.exit("\nPlease edit the script and set the correct folder name.")

    sys.exit(
        f"Could not find a North America folder beneath:\n{base}\n"
...
def main():
    ...
    north_america = find_north_america_folder(BASE)

    output_dir = north_america.parent / OUTPUT_NAME
    output_dir.mkdir(parents=True, exist_ok=True)

    files = [
        p for p in north_america.rglob("*")
    ...
    for src in sorted(files):
        ...
        copied += 1
        print(f"[{copied}/{len(files)}] {src.relative_to(north_america)}")
        print(f"    -> {dest.name}")
